function [nn,gam_val,w_est,gam_idx,f_hat_nm] = samp_sz(four_coef,Gam_vec,w_vec,s_vec,gam_mtx,~,epsTol,C,n0,samp_idx,nm_flg,w_flg,gam_val)
...
%       f_hat_pre = max(uell)/w_max; %smallest the norm can be
        ...
        %Compute sample size
        f_hat_nm = C * max(abs(four_coef(samp_idx)) ./ gam_val(samp_idx) ); % approx. f_hat
